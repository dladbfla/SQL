...
-- 단점 : 자니치게 복잡한 서브쿼리를 사용하면 성능을 저하시킬 수 있음
...
select 고객회사명, 담당자명 from 고객 inner join 주문 on 고객.고객번호 = 주문.고객번호 where 주문.주문번호 = 'H0250';
...
select 고객번호, 고객회사명 from 고객 where 고객번호 in (select 고객번호 from 주문);
select * from 주문;

-- join을 이용하여 동일한 결과 얻기
select 고객.고객번호, 고객회사명 from 고객 inner join 주문 on 고객.고객번호 = 주문.고객번호 group by 고객.	고객번호;
