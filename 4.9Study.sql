...
-- 비교연산자
select 23 >=5, 23<=5, 23>23, 23<23, 23=23, 23!=23, 23<>23; -- <> 같지 않다는 의미

-- 논리연산자 (and, or, not)
select * from 고객 where 도시 = '부산광역시' and 마일리지 < 1000;
select * from 고객 where 도시 = '부산광역시' or 마일리지 < 1000;
select * from 고객 where not 마일리지 < 1000;
...
-- 특수문자 %, _ 사용
select * from 고객 where 도시 like '%광역시'; -- %는 여러개의 문자가 존재할 수 있음을 의미
select * from 고객 where 도시 like '%광역%'; -- '광역'이 중간에 포함된 문자열
select * from 고객 where 도시 like '%시'; -- '시'로 끝나는 문자열
select * from 고객 where 고객번호 like '_C%'; -- 정확히 한 개의 문자를 의미. 두번째 자리가 'C'인 문자열
select * from 고객 where 고객번호 like '__C%'; -- 3번째가 'C'
...
select 제품번호, 제품명, 단가, 재고, 단가*재고 as 재고금액 from 제품 order by 5 desc limit 10; -- 숫자는 select의 컬럼순서(5= 재고금액)
